...
class DBG_Transport(DBG_AdapterBase):
        def __init__(self,spar):
                ...
                self.xx_dbg("DBG_Transport::__init__::m_in::")
                self.xx_set_class_name ( "Transport" )
                self.xx_set_full_class_name ( "iastora!Transport" )
                #self.m_child_item = DBG_ApstTable("Parent::DBG_Transport")
                ...
                self.xx_dbg("DBG_Transport::__init__::m_out::")
               
        def prepare_object(self):
                try:
                        self.prepare_object_internal(self)
                except:
                        self.xx_exception("DBG_Transport::prepare_object::exc::")
        ...
        def prepare_object_internal(self, pparent):
                ...
                self.xx_dbg("DBG_Transport::prepare_object::m_out::")
                
        def print_object(self):
                self.xx_dbg("DBG_Transport::print_object::m_in::")
                self.prepare_object()                
                self.xx_print_ptr("")
                if(self.xx_is_object() == 1):                
                        self.m_fields.xx_print_fields("")
                        #self.m_child_item.print_object()
                self.xx_dbg("DBG_Transport::print_object::m_out::")
