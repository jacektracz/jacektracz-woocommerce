...
class DBG_RaidMpbMgr(DBG_AdapterBase):
        def __init__(self,spar):
                ...
                self.xx_dbg("DBG_RaidMpbMgr::__init__::m_in::")
                self.xx_set_class_name ( "RaidMpbMgr" )
                self.xx_set_full_class_name ( "iastora!RaidMpbMgr" )
                #self.m_child_item = DBG_ApstTable("Parent::DBG_RaidMpbMgr")
                ...
                self.xx_dbg("DBG_RaidMpbMgr::__init__::m_out::")
               
        def prepare_object(self):
                try:
                        self.prepare_object_internal(self)
                except:
                        self.xx_exception("DBG_RaidMpbMgr::prepare_object::exc::")
        ...
        def prepare_object_internal(self, pparent):
                ...
                self.xx_dbg("DBG_RaidMpbMgr::prepare_object::m_out::")
                
        def print_object(self):
                self.xx_dbg("DBG_RaidMpbMgr::print_object::m_in::")
                self.prepare_object()                
                self.xx_print_ptr("")
                if(self.xx_is_object() == 1):                
                        self.m_fields.xx_print_fields("")
                        #self.m_child_item.print_object()
                self.xx_dbg("DBG_RaidMpbMgr::print_object::m_out::")
