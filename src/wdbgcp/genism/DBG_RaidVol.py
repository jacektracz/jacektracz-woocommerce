...
class DBG_RaidVol(DBG_AdapterBase):
        def __init__(self,spar):
                ...
                #self.m_child_item = DBG_ApstTable("Parent::DBG_RaidVol")
                self.m_fields = DBG_FieldsMapper("DBG_RaidVol"
                                         , self)
        ...
        def prepare_object_internal(self, pparent):
                self.xx_dbg("DBG_RaidVol::prepare_object_internal::")
                
                if(self.xx_is_object() == 1):
                        self.m_fields.set_fields_parent(self)                      
                        #self.m_child_item.set_addr_arr(self,"m_child_item")
                
                self.xx_dbg("DBG_RaidVol::prepare_object::m_out::")
