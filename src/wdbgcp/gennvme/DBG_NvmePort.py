...
class DBG_NvmePort(DBG_AdapterBase):
        def __init__(self,spar):
                ...
                self.xx_dbg("DBG_NvmePort::__init__::m_in::")
                self.xx_set_class_name ( "NvmePort" )
                self.xx_set_full_class_name ( "iastora!NvmePort" )
                #self.m_child_item = DBG_ApstTable("Parent::DBG_NvmePort")
                ...
                self.xx_dbg("DBG_NvmePort::__init__::m_out::")
               
        def prepare_object(self):
                try:
                        self.prepare_object_internal(self)
                except:
                        self.xx_exception("DBG_NvmePort::prepare_object::exc::")
        ...
        def prepare_object_internal(self, pparent):
                ...
                self.xx_dbg("DBG_NvmePort::prepare_object::m_out::")
                
        def print_object(self):
                self.xx_dbg("DBG_NvmePort::print_object::m_in::")
                self.prepare_object()                
                self.xx_print_ptr("")
                if(self.xx_is_object() == 1):                
                        self.m_fields.xx_print_fields("")
                        #self.m_child_item.print_object()
                self.xx_dbg("DBG_NvmePort::print_object::m_out::")
