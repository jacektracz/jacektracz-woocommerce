...
from ... appsrc.ismcfg.DBG_MpbDisk import *
from ... fields.DBG_FieldsMapper import *

class DBG_MpbDisk_PyArray(DBG_AdapterBase):
        def __init__(self,spar, pparent):
                DBG_AdapterBase.__init__(self, spar)
                self.xx_dbg("DBG_MpbDisk_PyArray::__init__::method_in::")
                ...
                self.m_tbl_sel_type = "pointer"
                self.init_object_fields()
                self.xx_dbg("DBG_MpbDisk_PyArray::__init__::method_out::")
        ...
        def set_parent_names(self,tt):
                self.xx_dbg("DBG_MpbDisk_PyArray::set_parent_names::method_in::")                                
                self.xx_set_class_name ( tt )
                self.xx_set_full_class_name ( "iastora!" + tt )
                self.xx_dbg("DBG_MpbDisk_PyArray::set_parent_names::method_out::")
        ...
        def set_parent(self,pparent):
                self.xx_dbg("DBG_MpbDisk_PyArray::set_parent::")
                self.m_parent = pparent
                self.xx_dbg("DBG_MpbDisk_PyArray::set_parent::out")
                
        def prepare_object(self,pparent):
                try:
                        self.prepare_object_internal(pparent)                                
                except:
                        self.xx_exception("DBG_MpbDisk_PyArray::print_object")
                                        
        def prepare_object_internal(self,pparent):
                self.xx_dbg("DBG_MpbDisk_PyArray::initialize_ptr::")
                self.m_parent = pparent                
                self.clear_messages()                
                ...
                self.xx_dbg("DBG_MpbDisk_PyArray::initialize_ptr::out")
                
        def prepare_array_items(self):                
                try:
                        self.xx_dbg("DBG_MpbDisk_PyArray::prepare_array_items::")
                        ...
                        self.xx_dbg("DBG_MpbDisk_PyArray::prepare_array_items::out")                                        
                except:
                        self.xx_exception("DBG_MpbDisk_PyArray::prepare_array_items::exception::")                                
                
        def get_array_item(self,p_ii):
                try:
                        self.xx_dbg("DBG_MpbDisk_PyArray::get_array_item::")
                        dd = DBG_MpbDisk("DBG_MpbDisk_PyArray",self)
                        s_selector = self.m_tbl_selector + "[" +str(p_ii) + "]"
                        
                        if (self.m_tbl_sel_type == "arr"):
                                dd.set_addr_arr(self,s_selector)
                        else:                                
                                dd.set_addr(self,s_selector)
                                
                        dd.prepare_object()
                        self.xx_dbg("DBG_MpbDisk_PyArray::get_array_item::out")
                        return dd
                except:
                        self.xx_exception("DBG_MpbDisk_PyArray::get_array_item::exception::")
                        return None
        ...
        def print_object_internal(self):
                self.xx_dbg("DBG_MpbDisk_PyArray::print_object::")
                self.xx_print_ptr("")
                self.m_fields.print_object()
                self.print_array_items()                
                self.xx_dbg("DBG_MpbDisk_PyArray::print_object::out")

        def print_array_items(self):
                self.xx_dbg("DBG_MpbDisk_PyArray::print_array_items::")
                for dd_item in self.m_array_items:
                        dd_item.print_object()
                self.xx_dbg("DBG_MpbDisk_PyArray::print_array_items::out::")
