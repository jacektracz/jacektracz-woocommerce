...
class DBG_CacheCandidate(DBG_AdapterBase):
        def __init__(self,spar, pparent):
                ...
                self.m_fields = DBG_FieldsMapper("DBG_CacheCandidate", self)
                self.init_object_fields()
                
                self.xx_dbg("DBG_CacheCandidate::__init__::m_out::")

        def init_object_fields(self):
                self.xx_dbg("DBG_CacheCandidate::init_object_fields::in::")                                
                #self.m_fields.add_fields_asstr_u32("mIntervalInMilliseconds")
                self.xx_dbg("DBG_CacheCandidate::init_object_fields::m_out::")
        ...
        def print_object(self,sdbg=""):
                try:
                        ...
                        self.xx_exception("DBG_CacheCandidate::print_object")
        ...
        def xx_obj_valid(self):
                self.xx_dbg("DBG_CacheCandidate::xx_obj_valid::m_in::")
                i_valid = self.xx_is_object()
                self.xx_dbg("DBG_CacheCandidate::xx_obj_valid::m_out::" + str(i_valid))
                return i_valid
