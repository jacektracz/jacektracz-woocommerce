...
class DBG_DbgGenericObjectEntryExt(DBG_AdapterBase):
        def __init__(self):
                DBG_AdapterBase.__init__(self,"DBG_DbgGenericObjectEntryExt")  
                self.m_exceptions = DBG_GlobalExceptionsHtmlPrinter(self,"Parent::DBG_DbgGenericObjectEntryExt")
                self.m_informations = DBG_GlobalInformationsHtmlPrinter(self,"Parent::DBG_DbgGenericObjectEntryExt")
                self.m_env_printer = DBG_EnvItemPrinter(self,"Parent::DBG_DbgGenericObjectEntryExt")
        ...
        def print_object(self):
                ...
                try:
                        ...
                        self.xx_exception("DBG_DbgGenericObjectEntryExt::prepare_object::excp")
                        
        def print_object_internal(self):
                try:
                        
                        self.xx_dbg("DBG_DbgGenericObjectEntryExt::print_object::in::")
                        ...
                        self.xx_dbg("DBG_DbgGenericObjectEntryExt::print_object::out::")
                        ...
                        try:
                                self.xx_exception_raw("DBG_DbgGenericObjectEntryExt::prepare_object::excp")
                        except:
                                self.xx_print_exception_raw("DBG_DbgGenericObjectEntryExt::prepare_object::excp")
                        return 0
        ...
        def str_to_class(self,module_name, class_name):
            try:
                ...
                if (inst_type == 1):
                        #mn = module_name
                        fpath = "driver.DBG_CrashHiberMemoryDescriptor"  
                ...
                if (inst_type == 2):
                        fpath = "D:\lkd\komodo\w2_2\src\w2\wdbgc\driver\DBG_CrashHiberMemoryDescriptor.py"
                        mn = "DBG_CrashHiberMemoryDescriptor.py"
                        cn = "DBG_CrashHiberMemoryDescriptor"
                        #mn = ".. driver.DBG_CrashHiberMemoryDescriptor"                                        
                        p = self.import_path(fpath)
                        m = getattr(p, mn)
                        c = getattr(m, cn)
                        dd_obj = c("",self)
                        return dd_obj
