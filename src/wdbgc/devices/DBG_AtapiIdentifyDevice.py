...
class DBG_AtapiIdentifyDevice(DBG_AdapterBase):
        def __init__(self,spar):                
                DBG_AdapterBase.__init__(self,spar)
                self.xx_dbg("DBG_AtapiIdentifyDevice::__init__::in::")
                self.xx_set_class_name ( "_ATAPI_IDENTIFY_PACKET_DATA" )
                self.xx_set_full_class_name ( "iastora!_ATAPI_IDENTIFY_PACKET_DATA" )
                self.m_fields = DBG_FieldsMapper("DBG_AtapiIdentifyDevice"
                                         ,self)                                
        ...
        def prepare_object_internal(self, pparent):
                self.xx_dbg("DBG_AtapiIdentifyDevice::prepare_object::")
                self.m_fields.set_fields_parent(self)
                self.xx_dbg("DBG_AtapiIdentifyDevice::prepare_object::out::")
        ...
        def create_init(self):
                try:
                        ...
                        self.m_fields.add_fields_str("modelNumber",40)
                        self.m_fields.add_fields_str("firmwareRevision",8)
        ...
        def print_object_internal(self,pparent):
                self.xx_dbg("DBG_AtapiIdentifyDevice::print_object_internal::")
                self.prepare_object()
                self.xx_print_ptr("")
                self.m_fields.xx_print_fields("")
                #if(self.mIdentifyControllerData.xx_is_object()):
                #        self.mIdentifyControllerData.print_object()
                self.xx_dbg("DBG_AtapiIdentifyDevice::print_object_internal::out::")                        
                        
                
        def get_class_str(self):
                self.xx_dbg("DBG_AtapiIdentifyDevice::get_class_str::")
